...
function normalizeFacebookPage(page) {
  if (!page?.id || !page?.access_token) {
    throw new FacebookApiError(
      "Facebook did not return a Page access token for a selected Page.",
    );
  }
...
export async function getFacebookPermissionStatuses(userAccessToken) {
...
export async function getFacebookPermissions(userAccessToken) {
  const statuses = await getFacebookPermissionStatuses(userAccessToken);
  return statuses
...
export async function listFacebookPages(userAccessToken) {
  ...
  return pages.map(normalizeFacebookPage);
}

export function getFacebookPageTargetIds(tokenDetails) {
  const pageScopes = new Set(REQUIRED_PERMISSIONS);
...
export async function listFacebookPagesByIds(userAccessToken, pageIds) {
  const results = await Promise.allSettled(
    pageIds.map(async (pageId) => {
      const page = await graphRequest(pageId, userAccessToken, {
        fields: "id,name,picture.type(square){url},access_token,tasks",
      });

      return normalizeFacebookPage(page);
    }),
  );
  ...
  return results.reduce(
    (result, item, index) => {
      if (item.status === "fulfilled") {
        result.pages.push(item.value);
      } else {
        result.errors.push({
          pageId: String(pageIds[index]),
          error: item.reason,
        });
      }
